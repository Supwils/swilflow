...
async changeAccentColorSetting(color: AccentColor) : Promise<Result<null, string>> {
    try {
...
async exportHistory(filePath: string, format: ExportFormat, filter: ExportFilter) : Promise<Result<number, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("export_history", { filePath, format, filter }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async transcribeAudioFile(filePath: string) : Promise<Result<TranscribeFileResult, string>> {
    try {
 ...
 */
async saveImportedTranscription(tempFileName: string, text: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("save_imported_transcription", { tempFileName, text }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async discardImportedTranscription(tempFileName: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("discard_imported_transcription", { tempFileName }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type ExportFilter = { type: "All" } | { type: "TimeRange"; from_timestamp: number; to_timestamp: number } | { type: "SelectedIds"; ids: number[] }
export type ExportFormat = "Csv" | "Markdown" | "Json"
export type GpuDeviceOption = { id: number; name: string; total_vram_mb: number }
export type HistoryEntry = { id: number; file_name: string; timestamp: number; saved: boolean; title: string; transcription_text: string; post_processed_text: string | null; post_process_prompt: string | null; post_process_requested: boolean }
export type HistoryStats = { total_entries: number; saved_entries: number; earliest_timestamp: number | null; latest_timestamp: number | null }
 ...
 */
export type TranscribeFileResult = { text: string; temp_file_name: string }
